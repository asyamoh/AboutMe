...
function beach(){
  ...
  case 'no':
  case 'n':
    alert('Oops you\'re wrong, who doesnt ?');
    break;
  default:
    alert('Please answer with yes/no or y/n');
    break;
...
function sibling(){
  for (let i = 0; i <4; i++) {
    let brothers = prompt('Guess How many brothers i have? ');
    brothers = Number();
    if (brothers === 3) { score++;
      alert('Bravo, your guessing is correct');
      break;
    } else if (0 < brothers && brothers < 2) {
...
function movie(){
  ...
  outerLoop: {
    for (let j = 1; j <= 6; j++) {
      let favMovie = prompt('What is my favorite type of movies do you think ?');
  ...
  alert(`My favorite movies types are ${typesOfMovies}`);
}
movie();
